...
def _load_manifest(path_value: str) -> tuple[str, str, Any, Path, str]:
    ...
    method_bundle = str(data.get("method_bundle") or "v2").strip().lower() or "v2"
    if method_bundle not in METHOD_BUNDLE_CHOICES:
...
def _build_method_bundle_comparison_summary(
    *,
    ...
    for cell_id in sorted(set(baseline_cells.keys()) | set(candidate_cells.keys())):
        ...
        model = (
            candidate_models.get(model_variant_id)
            or baseline_models.get(model_variant_id)
            or {}
        )
        method = (
        ...
        baseline_cell_stats = _build_methods_cell_stats(baseline_cell) if isinstance(baseline_cell, dict) else {
        ...
        candidate_cell_stats = _build_methods_cell_stats(candidate_cell) if isinstance(candidate_cell, dict) else {
            "micro": _prf_from_counts(0, 0, 0),
            "completed_count": 0,
            "failed_count": 0,
            "cancelled_count": 0,
        ...
        pairs.append(
            {
                ...
                baseline_bundle: {
                    key: value
                    for key, value in baseline_cell_stats.items()
                    if key != "documents"
                },
                candidate_bundle: {
                    key: value
                    for key, value in candidate_cell_stats.items()
                    if key != "documents"
                },
...
def run_methods_bundle_benchmark(args: argparse.Namespace) -> int:
    ...
    baseline_detail = create_methods_lab_run(
        body,
        session_id=args.session,
        run_async=False,
        method_bundle=args.baseline_bundle,
    )
    candidate_detail = create_methods_lab_run(
        body,
        session_id=args.session,
        run_async=False,
        method_bundle=args.candidate_bundle,
    )
    baseline_run = _load_methods_lab_run(str(baseline_detail["id"]), args.session)
    candidate_run = _load_methods_lab_run(str(candidate_detail["id"]), args.session)
    ...
    if args.output_dir:
        ...
        (output_dir / f"{prefix}_{args.baseline_bundle}.json").write_text(
            json.dumps(baseline_run, indent=2)
        )
        (output_dir / f"{prefix}_{args.candidate_bundle}.json").write_text(
            json.dumps(candidate_run, indent=2)
        )
